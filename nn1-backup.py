...
def process_data():
    ...
    #target = [9052,53802,48441,28525]
    #label += target
    #print("test:")
    #print("test:label")   
   # print(label.index((int('55650'))))
   # print(label.index((int('43543'))))
    ...
    #print(len(label))
    print("bitmap dimension" + str(bitmap.shape))
    fit_bitmap = np.unique(bitmap, axis=1)
    print("after unique" + str(fit_bitmap.shape))
    target_bitmap = np.zeros((len(seed_list),1))
    fit_bitmap = np.concatenate((fit_bitmap,target_bitmap),axis=1)
    n = fit_bitmap.shape[1]
...
def gen_mutate2(model, edge_num, sign):
    ...
    #interested_indice = [label.index((int('61961'))),label.index((int('1276'))),label.index((int('8651')))]
    interested_indice = [n-86,n-1,n-587]
    print("n"+str(n))
    print("test:interested_indice")
    #print(label[interested_indice[0]])
    #print(label[interested_indice[1]])
    #print(label[interested_indice[2]])
    ...
    with open('gradient_info_p', 'w') as f:
	    ...
	    for idx in range(len(rand_seed1[:])):
                fl = [rand_seed1[idx], rand_seed2[idx]]
...
def build_model():
    ...
    global checkpoint
    checkpoint = ModelCheckpoint("trained_best_weights.h5",monitor='val_loss', save_weights_only=True,verbose=1,save_best_only=True, period=1)
    model.summary()
...
def train(model):
    ...
    if os.path.exists("trained_best_weights.h5"):
    	model.load_weights("trained_best_weights.h5")
...
def gen_grad(data):
    ...
    train(model)
    # model.load_weights('hard_label.h5')
    gen_mutate2(model, 3, data[:5] == b"train")
